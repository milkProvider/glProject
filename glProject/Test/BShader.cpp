...
*/
BShader::BShader(const char *vsFile, const char *fsFile) {
...
*/
void BShader::init(const char *vsFile, const char *fsFile) {
	...
	int vertex_shader_string_length = vsText.length();
	int fragment_shader_string_length = vsText.length();

    glShaderSource(shader_vp, 1, &vertexText, &vertex_shader_string_length); // Set the source for the vertex shader to the loaded text
    glCompileShader(shader_vp); // Compile the vertex shader
    validateShader(shader_vp, vsFile); // Validate the vertex shader
    
    glShaderSource(shader_fp, 1, &fragmentText, &fragment_shader_string_length); // Set the source for the fragment shader to the loaded text
	...
	glBindAttribLocation(shader_id, 0, "vertex"); // Bind a constant attribute location for positions of vertices
	glBindAttribLocation(shader_id, 1, "uv");
	glBindAttribLocation(shader_id, 2, "normal");
	glBindAttribLocation(shader_id, 3, "bone_ids");
	glBindAttribLocation(shader_id, 4, "bone_weights");
	...
	MVP_loc = glGetUniformLocation(shader_id, "MVP");
	bones_loc = glGetUniformLocation(shader_id, "bones");

	GLuint lTex = glGetUniformLocation(shader_id, "_MainTex");
	glUniform1i(lTex, 0);
}
...
void BShader::setMVPMatrix(glm::mat4x4 pMVPMatrix)
{
	bind();
	glUniformMatrix4fv(MVP_loc, 1, GL_FALSE, glm::value_ptr(pMVPMatrix));
	unbind();
}

void BShader::setBoneMatrix(glm::mat4x4 pBones[61])
{
	//bind();
	glUniformMatrix4fv(bones_loc, 61, GL_FALSE, glm::value_ptr(pBones[0]));
	//unbind();
}

void BShader::updateBoneMatrix(int pIndex, glm::mat4x4 pMatrix)
{
